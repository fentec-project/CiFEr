...
void cfe_damgard_multi_master_keys_init(cfe_mat *mpk, cfe_damgard_multi_sec_key *msk, cfe_damgard_multi *m) {
    msk->msk = cfe_malloc(sizeof(cfe_damgard_sec_key) * m->num_clients);
    msk->num_keys = m->num_clients;
    cfe_mat_inits(m->num_clients, m->scheme.l, mpk, &msk->otp, NULL);
}
...
void cfe_damgard_multi_generate_master_keys(cfe_mat *mpk, cfe_damgard_multi_sec_key *msk, cfe_damgard_multi *m) {
    ...
    for (size_t i = 0; i < m->num_clients; i++) {
...
void cfe_damgard_multi_fe_key_init(cfe_damgard_multi_fe_key *fe_key, cfe_damgard_multi *m) {
    fe_key->num_clients = m->num_clients;
    mpz_init(fe_key->z);
    fe_key->keys = (cfe_damgard_fe_key *) cfe_malloc(sizeof(cfe_damgard_fe_key) * m->num_clients);
    for (size_t i = 0; i < m->num_clients; i++) {
...
void cfe_damgard_multi_fe_key_free(cfe_damgard_multi_fe_key *key) {
    for (size_t i = 0; i < key->num_clients; i++) {
        cfe_damgard_fe_key_free(&(key->keys[i]));
    }
    free(key->keys);
    mpz_clear(key->z);
}

cfe_error cfe_damgard_multi_derive_fe_key(cfe_damgard_multi_fe_key *fe_key, cfe_damgard_multi *m,
    ...
    for (size_t i = 0; i < m->num_clients; i++) {
        err = cfe_damgard_derive_fe_key(&(fe_key->keys[i]), &m->scheme, &msk->msk[i], cfe_mat_get_row_ptr(y, i));
        if (err) {
...
cfe_error cfe_damgard_multi_decrypt(mpz_t res, cfe_damgard_multi *m, cfe_vec *ciphertext, cfe_damgard_multi_fe_key *fe_key,
    ...
    mpz_mul_ui(bound, bound, m->num_clients*m->scheme.l);
